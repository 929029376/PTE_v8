...
class FocalLoss(nn.Module, ABC):
    ...
    def forward(self, prediction, target):
        # In bfloat16, 1 - 1e-4 rounds back to 1.0. Compute the logarithms in
        # float32 and clamp both ends so saturated heatmaps remain finite.
        prediction = prediction.float().clamp(1e-4, 1.0 - 1e-4)
        target = target.float()
