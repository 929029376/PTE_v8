...
class THOR_Wrapper():
    def __init__(self, net, st_capacity, lt_capacity, sample_interval, update_interval, lower_bound=0.90, score_threshold=0.7):
        ...
        self.frozen = False
        # Clean template snapshot (the raw image patches at FROZEN entry),
        # used by the redetection expert to match against an uncorrupted
        # template rather than the drifting dynamic one.
        self.clean_zi = None
        self.clean_ze = None
    ...
    def snapshot_clean(self, zi, ze):
        """Capture a clean template at the moment of entering FROZEN. This is
        the template the redetection expert matches against; it is never
        updated during the occlusion episode."""
        self.clean_zi = zi.detach().clone()
        self.clean_ze = ze.detach().clone()

    def get_clean_template(self):
        """Return (clean_zi, clean_ze) for redetection matching, or None if no
        snapshot has been taken."""
        if self.clean_zi is None:
            return None
