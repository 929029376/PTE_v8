...
class PETBackBone(VisionTransformer):
...
def _create_pet_vit(pretrained=False, **kwargs):
    model = PETBackBone(**kwargs)
    if pretrained:
...
def pet_vit_base_patch16_224(pretrained=False, **kwargs):
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_pet_vit(pretrained=pretrained, **model_kwargs)
    return model
