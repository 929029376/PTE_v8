...
def _full_frame_anchor(frame, search_area_factor, reference_box):
    """Box whose factor-scaled square crop covers the complete frame."""
    if torch.is_tensor(frame):
        if frame.dim() >= 3 and frame.shape[-1] in (1, 3, 4):
            height, width = frame.shape[-3], frame.shape[-2]
        else:
    ...
    side = float(max(height, width)) / float(search_area_factor)
    return reference_box.new_tensor([
        0.5 * (float(width) - side),
        0.5 * (float(height) - side),
        side,
        side,
    ])
