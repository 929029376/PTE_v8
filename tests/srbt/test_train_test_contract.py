...
def test_canonical_config_contains_local_experts_but_no_legacy_pet_or_c3_nodes():
    ...
    assert configured.MODEL.INIT_CHECKPOINT.endswith(
        "causal_event_motion_exclusive_v35_20260721/checkpoints/train/pet_track/"
...
def test_canonical_causal_motion_strategy_trains_only_motion_specialist():
    ...
    assert configured.TRAIN.SMALL_TARGET_SOFT_BOX_TEMPERATURE == pytest.approx(
        0.2)
    assert configured.TRAIN.LR_DROP_EPOCH == 20
...
def test_supervisor_uses_causal_event_motion_jitter_v36_run_directory():
    project_root = Path(__file__).resolve().parents[2]
    supervisor = (
        project_root / "tracking" / "supervisord_local_experts_v8.conf"
    ).read_text(encoding="utf-8")

    assert "causal_event_motion_jitter_v36_20260721" in supervisor
    ...
    assert supervisor.count(
        ...
        "causal_event_motion_jitter_v36_20260721/logs && exec") == 2
