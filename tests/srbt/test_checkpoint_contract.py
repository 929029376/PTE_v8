...
def _motion_temporal_keys(model):
    return sorted(
        key for key in model.state_dict()
        if key.startswith("expert_fusion.experts.motion_fm.temporal_")
    )
...
def test_v27_checkpoint_retains_old_weights_and_initializes_motion_history(
        tmp_path):
    from tests.srbt.test_srbt_model_integration import _model
    ...
    temporal_before = {
        key: model.state_dict()[key].clone() for key in temporal_keys
    }
    source = {
        key: torch.full_like(value, 0.625)
        for key, value in model.state_dict().items()
        if key not in temporal_before and key != "_pet_architecture_version"
    }
    source["_pet_architecture_version"] = torch.tensor(27)
    checkpoint = tmp_path / "v27-motion-history.pth.tar"
    torch.save({"net": source}, checkpoint)

    report = _load_retained_model_checkpoint(
        model, checkpoint, label="v27 motion-history migration")
    ...
    assert all(
        torch.equal(loaded[key], value)
        for key, value in temporal_before.items()
    )
    assert report["initialized_extension_keys"] == temporal_keys


def test_v28_checkpoint_retains_all_weights_and_initializes_only_localization_gate(
        tmp_path):
    from tests.srbt.test_srbt_model_integration import _model
    ...
    assert gate_before
    source = {
        key: torch.full_like(value, 0.8125)
        for key, value in model.state_dict().items()
        if key not in gate_before and key != "_pet_architecture_version"
    }
    source["_pet_architecture_version"] = torch.tensor(28)
    checkpoint = tmp_path / "v28-dart-reliability-migration.pth.tar"
    torch.save({"net": source}, checkpoint)

    report = _load_retained_model_checkpoint(
        model, checkpoint, label="v28 DART reliability migration")
...
def test_v30_checkpoint_initializes_only_expert_collaboration_gate(tmp_path):
    from tests.srbt.test_srbt_model_integration import _model
    ...
    collaboration_before = {
        key: value.clone() for key, value in model.state_dict().items()
        if key.startswith("expert_collaboration_gate.")
    }
    assert collaboration_before
    source = {
        key: torch.full_like(value, 0.5625)
        for key, value in model.state_dict().items()
        if key not in collaboration_before and key != "_pet_architecture_version"
    }
    source["_pet_architecture_version"] = torch.tensor(30)
    checkpoint = tmp_path / "v30-collaboration-gate-migration.pth.tar"
    torch.save({"net": source}, checkpoint)

    report = _load_retained_model_checkpoint(
        model, checkpoint, label="v30 collaboration migration")
    ...
    assert all(
        torch.equal(loaded[key], value)
        for key, value in collaboration_before.items()
    )
